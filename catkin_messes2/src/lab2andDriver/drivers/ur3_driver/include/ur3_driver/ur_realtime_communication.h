...
private:
	...
	void run();
	ur3_driver::position	pos_msg; //actual message for the position publishing to the student.
						//student will use this to figure out the current state of the robot.
...
public:
	...
	UrRealtimeCommunication(std::condition_variable& msg_cond, std::string host,
			ros::NodeHandle nharg,unsigned int safety_count_max = 12);
	bool start();
	void halt();
