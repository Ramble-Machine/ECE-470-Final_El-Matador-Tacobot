...
UrCommunication::UrCommunication(std::condition_variable& msg_cond,
		std::string host, ros::NodeHandle nhparameter) {
	...
	input_pub=nh.advertise<ur3_driver::gripper_input>("ur3/gripper_input",10);
}
...
void UrCommunication::run() {
	...
	ROS_INFO("Secondary port: Got connection");
	while (keepalive_) {
		while (connected_ && keepalive_) {
			...
			if (bytes_read > 0) {
				...
				dig_inputs = robot_state_->getDigitalInputBits();
				analog0 = robot_state_->getAnalogInput0();
				analog1 = robot_state_->getAnalogInput1();
				
				input_msg.DIGIN=dig_inputs;
				input_msg.AIN0 = analog0;
				input_msg.AIN1 = analog1;
				input_pub.publish(input_msg);

//				std::cout << "DigIn = " << dig_inputs;
//				std::cout << " Analog0 = " << analog0;
//				std::cout << " Analog1 = " << analog1 << "\n";
